...
The results are summarized in the following histogram (256 bins), which contains 126,872 data samples with *all* angles. Notice that very high frequencies are randomly removed to avoid a possible bias.
...
As useful data is limited (even with data augmentation), the car controller has to be simple enough for avoiding high bias (that is: overfitting) and at the same time deep enough for generalizing properly. Taking these restrictions into account, and after carefully reading Nvidia's paper (https://devblogs.nvidia.com/parallelforall/deep-learning-self-driving-cars/) and Mishkin et al. paper (https://arxiv.org/abs/1606.02228), the final model utilizes a LeNet-alike pattern but adding some extra layers for improving the car's behavior. Readers may find it similar to a simplified VGGNet architecture (https://arxiv.org/abs/1409.1556). The model is depicted in what follows.
...
Notice that SimpLeNet does not use fully connected layers. This way the model benefits of having less parameters while showing a similar performance. Notice that the we pass from about 1,500,000+ parameters using fully connected layers to about 134,000 parameters by using 1x1 convolutions and global average pooling, as suggested by Mishkin et al. paper (2016).

In summary, the architecture follows the pattern: *[INPUT]->[CONV->LeakyReLU->CONV->LeakyReLU]->[CONV->ELU->CONV->ELU->MAXPOOLING->DROPOUT]{2}->[CONV->ELU->DROPOUT->CONV->ELU->FC]*. Notice that this pattern is the recommended in http://cs231n.github.io/convolutional-networks/.

The total number of layers are:
* 8 Convolution layers (5x5, 3x3 and 1x1).
* 1 Fully connected layer (1 unit).
* 2 (Very) Leaky ReLU layers.
* 6 ELU layers.
* 3 Maxpooling layers (strides (2, 3), (2, 2), and (2, 4)).
* 1 Global average pooling.
* 3 Dropout layers (0.25, 0.4 and 0.5 **dropout** probability).

In total 24 layers and 133,226 trainable parameters.
...
As future work lines we may (1) use distinct models and ensemble them for obtaining a richer car controller, (2) use the already trained model to follow a reinforcement learning stage to fine tune the driving, and (3) use the trained model in a real-world RC car by attaching a camera to the device.
